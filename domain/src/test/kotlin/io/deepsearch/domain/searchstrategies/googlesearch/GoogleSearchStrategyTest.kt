...
class GoogleSearchStrategyTest : KoinTest {
    ...
    @Test
    fun `basic summary on example dot com`() = runTest {
        // Given
    ...
    @Test
    fun `pricing and sla question on vendor site`() = runTest {
        // Given
    ...
    @Test
    fun `clearly unrelated query still produces grounded answer using given site`() = runTest {
        // Given
        val url = "https://www.example.com/"
        val searchQuery = SearchQuery(
    ...
    @Test
    fun `non english query on example dot com`() = runTest {
        // Given
        val url = "https://www.example.com/"
        val searchQuery = SearchQuery(
            query = "這個網站的主要內容是什麼？",
